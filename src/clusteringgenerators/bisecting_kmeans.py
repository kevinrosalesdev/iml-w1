...
class BisectingKMeans:
    def __init__(self, n_clusters: int, selector_type: str):
        ...
        accepted_selector_type = ['dimension', 'heterogeneous']
    ...
    def apply_unsupervised_learning(self, X):
        ...
        sub_dataset_map[0] = branch_to_divide
        i = 0
        print("Applying Bisecting Kmeans clustering...")
        while len(sub_dataset_map) < self.n_clusters:
            if self.selector_type == 'dimension':
                branch_to_divide = self.get_biggest_cluster(sub_dataset_map)
            ...
            # print(i in sub_dataset_map.keys())
            sub_dataset_map[i] = left_branch
            # print("left_branch =", len(left_branch.index), "key = ", i)
            i += 1
            # print(i in sub_dataset_map.keys())
            sub_dataset_map[i] = right_branch
            # print("right_branch =", len(right_branch.index), "key = ", i)
            i += 1

        # print(f"END WHILE --> Number of clusters found = {len(sub_dataset_map)}")

        predictions = pd.Series(np.zeros(len(X), dtype=int), index=X.index)
        keys = list(sub_dataset_map.keys())
        for i in range(len(sub_dataset_map.keys())):
            key = keys[i]
            subset = sub_dataset_map[key]
            predictions[predictions.index.isin(subset.index)] = i
            # print("cluster", i, "-->", len(subset))
        return predictions.values

    def get_biggest_cluster(self, sub_dataset_map):
        largest_value = 0
        largest_key = 0
        for key, value in sub_dataset_map.items():
            num_items = len(value.index)
            if num_items >= largest_value:
                largest_value = num_items
                largest_key = key
        branch_to_divide = sub_dataset_map[largest_key]
        sub_dataset_map.pop(largest_key)
